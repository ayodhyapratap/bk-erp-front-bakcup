...
export default class MlmBase extends React.Component {
  ...
  render(){
          ...
          if(that.state.productLevels){
            that.state.productLevels.forEach(function (level){
              if(that.state.mlmItems){
                that.state.mlmItems.forEach(function (item){
                  if(item.id==role.id &&item.level==level.id){
    ...
    return <div>
        <Switch>
            <Route path="/mlm/generate" render={(route) => <MLMGenerate {...route} {...this.state}/>}/>
            <Route>
                <Card title="MLM List"
                      extra={<div>
                        <Link to="/mlm/generate"><Button><Icon type="plus"/> Generate MLM</Button></Link>
                      </div>}>
